...
function AtomSetView( mol, coords, dir ) {
	...
	var negate = 1;
	var flip = 0;
	if(dir=='backview' || dir=='leftview' || dir=='downview'){negate = -1; flip = Math.PI;}
	
	if(dir=='frontview' || dir=='backview'){
		if(coords[0] == 0 && coords[1] == 0){
			if(negate*coords[2] < 0){
				mat4.rotate(newRotationMatrix, Math.PI, [1,0,0] );}}
		else{
			if(coords[2] == 0){
				var degree = Math.PI/2;}
			else{
				var degree = Math.atan2( Math.sqrt( coords[0]*coords[0] + coords[1]*coords[1] ), coords[2] );}
			mat4.rotate(newRotationMatrix, degree+flip, [coords[1],-coords[0],0]);}}
	
	if(dir=='rightview' || dir=='leftview'){
		if(coords[1] == 0 && coords[2] == 0){
			if(negate*coords[0] < 0){
				mat4.rotate(newRotationMatrix, Math.PI, [0,1,0] );}}
		else{
			if(coords[0] == 0){
				var degree = Math.PI/2;}
			else{
				var degree = Math.atan2( Math.sqrt( coords[1]*coords[1] + coords[2]*coords[2] ), coords[0] );}
			mat4.rotate(newRotationMatrix, degree+flip, [0,coords[2],-coords[1]]);}}
			
	if(dir=='upview' || dir=='downview'){
		if(coords[2] == 0 && coords[0] == 0){
			if(negate*coords[1] < 0){
				mat4.rotate(newRotationMatrix, Math.PI, [0,0,1] );}}
		else{
			if(coords[1] == 0){
				var degree = Math.PI/2;}
			else{
				var degree = Math.atan2( Math.sqrt( coords[2]*coords[2] + coords[0]*coords[0] ), coords[1] );}
			mat4.rotate(newRotationMatrix, degree+flip, [-coords[2],0,coords[0]]);}}
	...
	var NoAtomos = molecule.LstAtoms.length;
			...
			if (NoBloques==1)
			{
				...
				for(var i=0; i<NoAtomos; i++)
				{
					...
					if( molecule.focusOn.length == molecule.LstAtoms.length || molecule.focusOn.includes(i) )
					{
				...
				for(var i=0; i<NoBloques; i++)
				{
					...
					if (i==NoBloques-1) //esto es que llegó al último
					{
						for(var j=0; j<Restantes; j++)
						{
							...
							if( molecule.focusOn.length == molecule.LstAtoms.length || molecule.focusOn.includes(apuntador) )
							{
								//la molecula tiene la lista de átomos en orden, entonces lo recorro con los i
