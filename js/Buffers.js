...
function initBuffersBonds(Prendidos)
{
    ...
    for(var t in molecule.LstBonds)
    {
		...
		if( molecule.focusOn.length == molecule.LstAtoms.length || molecule.focusOn.includes(atm_0.id) && molecule.focusOn.includes(atm_1.id) )
		{	
						//-----------------------------------------------------------------------------------------
						if (DinamicaActiva)  //////////*********** checar instrucción
						{
...
function initBufferSpline() {//Julio
    ...
    for (var t in molecule.LstBondsSkeleton) {
		...
		if( molecule.focusOn.length == molecule.LstAtoms.length || molecule.focusOn.includes(atm_0.id) )
		{
			if (PsAntA !== undefined)
