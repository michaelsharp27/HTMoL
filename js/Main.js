...
//alert(prr.replace(/[ ,]+/g, ","));
    ...
    //--------------------------
    full_molecule=this.ObjP.ReadFile(PDBDIR+pdbInicial);
...
//        info.innerHTML="HTMoLv3.5: "+url+" ("+molecule.LstAtoms.length+" atoms)";
    ...
    this.Parse=function(txt) // Parsing commands in the console
    {
		...
		else if (comando=='rotate') /*	[MRS] data stored in coordsX, coordsY, coordsZ
										stored all x coords for atoms 1-3549 for frame 1 ->
										all x coords for atoms 1-3549 for frame 2 -> and so on 
										also stored in molecule.LstAtoms, must be relative to molecule.Center[X,Y,Z] */
		{
			var comando=txt.substr(0, txt.indexOf(" ")).toLowerCase();
			var dir = inst.substr( inst.indexOf(" ")+1, inst.length ).toLowerCase();
			inst = inst.substr( 0, inst.indexOf(" ") );
			var coords = [molecule.LstAtoms[inst].X-molecule.CenterX, molecule.LstAtoms[inst].Y-molecule.CenterY, molecule.LstAtoms[inst].Z-molecule.CenterZ]
			console.log( 'centering atom ' + inst + ' to view ' + dir );
			AtomSetView(molecule, coords, dir);
		}
		else if (comando=='center') /*	[MRS] make a focusOn attribute that holds the indicies of atoms
										that camera will center around, used in MainHTMoL script drawScene() */
		{
			if( inst == "all" ){molecule.focusOn = range(0, molecule.LstAtoms.length-1);}
				
			else{
				
				if(inst.indexOf(":") == -1 && !isNaN(inst)){
					molecule.focusOn = [ parseInt(inst) ];}
					
				else if( inst.indexOf(":") != -1 && !isNaN(inst.substr( 0, inst.indexOf(":") )) && !isNaN(inst.substr( inst.indexOf(":")+1, inst.length )) ){
					inst1 = inst.substr( 0, inst.indexOf(":") );
					inst2 = inst.substr( inst.indexOf(":")+1, inst.length );
					molecule.focusOn = range(parseInt(inst1),parseInt(inst2));}
					
				else{document.getElementById("Console_output").value="Unknown command : " + inst;}}
			
			//alert("inst = " + inst + "\tinst == \"all\" evaluates to " + (inst == "all"))
			//alert("Focus set to : " + molecule.focusOn)
			
			// molecule.CenterX = 0;
			// molecule.CenterY = 0;
			// molecule.CenterZ = 0;
			
			// for (var i in molecule.focusOn)
			// {
				// molecule.CenterX += molecule.LstAtoms[i].X;
				// molecule.CenterY += molecule.LstAtoms[i].Y;
				// molecule.CenterZ += molecule.LstAtoms[i].Z;
			// }
			
			// molecule.CenterX = molecule.CenterX / molecule.focusOn.length;
			// molecule.CenterY = molecule.CenterY / molecule.focusOn.length;
			// molecule.CenterZ = molecule.CenterZ / molecule.focusOn.length;
		}
        else
        {
            document.getElementById("Console_output").value="Unknown command : " + txt;
        }
