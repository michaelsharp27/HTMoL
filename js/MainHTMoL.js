...
//var URL_TRJ_AutoLoad_default=PDBDIR+"prueba.pdb";
    ...
    function drawScene(diffuse)
    {
        ...
        if(trjbnd && autoplay) //trjbnd y autoplay están declaradas en el Main
        {
			...
			/* [MRS] adjusted coordinates so that it centers around atoms with "focusOn" designation */
			var temp = parseInt(molecule.LstAtoms.length*pos);
			
			//var alreadyAlerted = false;
			//var count = 0
			//var str = "";
			
            for (var i = 0; i < molecule.focusOn.length; i++)
            {
				atom = molecule.focusOn[i];
				if(temp <= coordsX.length){
					x1 += coordsX[parseInt(atom)+parseInt(temp)];
					y1 += coordsY[parseInt(atom)+parseInt(temp)];
					z1 += coordsZ[parseInt(atom)+parseInt(temp)];
					//if(!alreadyAlerted && !(coordsX[i+temp] >= 0) ){alert("i = " + i + "\ttemp = " + temp + "\tcoordsX = " + coordsX[i+temp-1] + ", " + coordsX[i+temp] + ", " + coordsX[i+temp+1]); alreadyAlerted=true}
					//str += ( "atom = " + atom + ": (" + coordsX[parseInt(atom)+parseInt(temp)] + "," + coordsY[parseInt(atom)+parseInt(temp)] + "," + coordsZ[parseInt(atom)+parseInt(temp)] + ")\n" );
				}
				else{
					x1 += coordsX1[parseInt(atom)+parseInt(temp)];
					y1 += coordsY1[parseInt(atom)+parseInt(temp)];
					z1 += coordsZ1[parseInt(atom)+parseInt(temp)];
					//if(!(coordsX1[i+temp] >= 0) ){alert("i = " + i + "\ttemp = " + temp + "\tcoordsX1 = " + coordsX[i+temp]);}
					//str += ( "atom = " + atom + ": (" + coordsX1[parseInt(atom)+parseInt(temp)] + "," + coordsY1[parseInt(atom)+parseInt(temp)] + "," + coordsZ1[parseInt(atom)+parseInt(temp)] + ")\n" );
				}
            }
			
			//if (count < 10){alert(str);}
			
            Cx=x1/molecule.focusOn.length;
            Cy=y1/molecule.focusOn.length;
            Cz=z1/molecule.focusOn.length;
			
			//alert("temp = " + temp + "\tcoordsX.length = " + coordsX.length + "\tcoordsX1.length = " + coordsX1.length);
			//alert("x1 = " + x1 + "\tmolecule.focusOn.length = " + molecule.focusOn.length + "\tCx = " + Cx);
            ...
            for (var i=0; i<molecule.LstBonds.length;i++)
            {
				...
				if( molecule.focusOn.length == molecule.LstAtoms.length || ( molecule.focusOn.includes(atm1.id) && molecule.focusOn.includes(atm2.id) ) )
				{
